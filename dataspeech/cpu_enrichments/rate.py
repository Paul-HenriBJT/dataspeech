...
def rate_apply(batch, rank=None, audio_column_name="audio", text_column_name="text"):
    if isinstance(batch[audio_column_name], list):  
        ...
        for text, audio in zip(batch[text_column_name], batch[audio_column_name]):
            phonemes = phonemize(text, language='fr-fr', backend='espeak', with_stress=True)
        ...
        phonemes = phonemize(batch[text_column_name], language='fr-fr', backend='espeak', with_stress=True)
